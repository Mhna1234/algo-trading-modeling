...
This module provides comprehensive portfolio management components that seamlessly
integrate forecasting, signals, and backtesting into a unified system.

Key Features:
- Forecast-driven portfolio optimization with ARIMA-GARCH models
- Signal-based portfolio construction and weighting
- Advanced backtesting with realistic cost modeling
- Unified configuration management across all components

Management Components:
- ConfigManager: Handles configuration and parameter translation
- ForecastManager: Integrates forecasting with portfolio optimization
...
This module replaces the portfolio_adapter.py with more intuitive naming
and better organization for continuous workflow integration.
"""
...
class BacktestResult:
    """
    Comprehensive backtest result container with full performance analytics.
    
    This provides a unified interface for accessing all backtest metrics,
    ...
    def summary(self) -> Dict[str, float]:
        """Return comprehensive summary dictionary of all metrics."""
        return {
...
class ConfigManager:
    """
    Central configuration manager for portfolio system parameters.
    
    Handles translation between TradingConfig and various component parameters,
    ensuring consistency across the entire portfolio management pipeline.
    """
    
    @staticmethod
    def trading_config_to_portfolio_params(config: TradingConfig) -> Dict[str, Any]:
        """
        Convert TradingConfig to Portfolio initialization parameters.
...
class ForecastManager:
    """
    Forecast-driven portfolio management system.
    
    Integrates ARIMA-GARCH forecasting with portfolio optimization to generate
    dynamic, forward-looking portfolio weights based on predicted returns and volatility.
    """
    ...
    def __init__(self, 
        ...
        self.config = config
        self.optimization_params = ConfigManager.trading_config_to_optimization_params(config)
        
        logger.info(f"Initialized ForecastManager with method: {config.optimization_method}")
    
    def create_forecast_rule(self, 
                           forecaster,
                           method: Optional[str] = None) -> Callable[[pd.Timestamp, pd.Series], pd.Series]:
        """
        Create a portfolio construction rule using ARIMA-GARCH forecasts.
        ...
        def forecast_rule(date: pd.Timestamp, past_returns: pd.Series) -> pd.Series:
            """
            Portfolio construction rule using forecasts for optimal weights.
            ...
            Returns:
                Optimal portfolio weights based on forecasts
            """
            try:
                # Get data window for forecasting
                ...
                if opt_method == 'sharpe' or opt_method == 'tangency':
                    ...
                    # Target return based on expected returns
    ...
    def generate_weights_from_forecasts(self,
        ...
        Args:
            forecaster: ARIMAGARCHForecaster instance
            schedule: Rebalancing schedule ('D', 'W', 'M', 'Q')
...
class SignalManager:
    """
    Signal-based portfolio management system.
    
    Converts trading signals from technical analysis or other sources into
    portfolio weights for execution and backtesting.
    """
    ...
    def create_signal_rule(self, 
        ...
        """
        Create portfolio construction rule from trading signals.
...
class PortfolioBacktester:
    """
    Production-ready portfolio backtesting system.
    
    Provides comprehensive backtesting with realistic transaction costs, slippage,
    benchmark comparison, and full performance analytics.
    """
    ...
    def __init__(self, config: TradingConfig):
        ...
        self.config = config
        self.portfolio = None
        self.portfolio_params = ConfigManager.trading_config_to_portfolio_params(config)
        
        logger.info("Initialized PortfolioBacktester")
    
    def run_backtest(self,
                    price_data: pd.DataFrame,
                    weight_data: pd.DataFrame,
                    benchmark_data: Optional[pd.DataFrame] = None,
                    signals_data: Optional[pd.DataFrame] = None) -> BacktestResult:
        """
        ...
        Args:
            ...
            signals_data: Trading signals (optional, for reference)
            
        Returns:
            BacktestResult with complete performance analytics
        """
        logger.info("Running backtest via PortfolioBacktester")
    ...
    def _create_backtest_result(self,
                             portfolio_result: PortfolioResult,
                             benchmark_data: Optional[pd.DataFrame],
                             date_index: pd.DatetimeIndex) -> BacktestResult:
        """
        Create comprehensive BacktestResult with all analytics.
...
AdapterResult = BacktestResult
ConfigurationAdapter = ConfigManager
ForecastPortfolioAdapter = ForecastManager
SignalPortfolioAdapter = SignalManager
BacktesterAdapter = PortfolioBacktester
