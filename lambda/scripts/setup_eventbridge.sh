...
for i in "${!PARTITIONS[@]}"; do
    ...
    RULE_EXISTS=$(aws.exe events describe-rule --name ${RULE_NAME} --region ${AWS_REGION} 2>&1 || echo "NOT_FOUND")

    if [[ "$RULE_EXISTS" == *"NOT_FOUND"* ]]; then
        echo "  1. Creating EventBridge rule..."
        aws.exe events put-rule \
            --name ${RULE_NAME} \
...
echo "  aws.exe events list-rules --name-prefix benchmark-daily-trigger --region $AWS_REGION"
echo ""
echo "To test manually:"
echo "  aws.exe lambda invoke --function-name benchmark-calculator-partition-1 --region $AWS_REGION response1.json"
echo "  aws.exe lambda invoke --function-name benchmark-calculator-partition-2 --region $AWS_REGION response2.json"
echo "  aws.exe lambda invoke --function-name benchmark-calculator-partition-3 --region $AWS_REGION response3.json"
